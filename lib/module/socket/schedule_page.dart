...
class ScheduleSocket extends StatefulWidget {
  const ScheduleSocket({super.key, required this.token});
  final String token;

  @override
  State<ScheduleSocket> createState() => _ScheduleSocketState();
}

class _ScheduleSocketState extends State<ScheduleSocket> {
      ...
      children: [
        ...
        for (int index = 0; index < days.length; index++)
          Builder(builder: (context) {
            int scheduleIndex =
                socketData.schedules.indexWhere((e) => e.day == days[index]);
            return Container(
              ...
              child: ExpansionTile(
                ...
                children: [
                  if (scheduleIndex != -1)
                    for (int i = 0;
                        i < socketData.schedules[scheduleIndex].actions.length;
                        i++)
                      ListTile(
                        contentPadding:
                            const EdgeInsets.only(left: 65, right: 65),
                        title: Text(
                            socketData.schedules[scheduleIndex].actions[i].time,
                            style: const TextStyle(
                                fontSize: 20, color: Colors.black)),
                        subtitle: Text(
                          socketData.schedules[scheduleIndex].actions[i].action
                              ? "on".tr
                              : "off".tr,
                        ),
                        trailing: IconButton(
