...
class ScheduleGate extends StatefulWidget {
  const ScheduleGate({super.key, required this.token});
  final String token;

  @override
  State<ScheduleGate> createState() => _ScheduleGateState();
}

class _ScheduleGateState extends State<ScheduleGate> {
      ...
      children: [
        ...
        for (int index = 0; index < days.length; index++)
          Builder(builder: (context) {
            int scheduleIndex =
                gateData.schedules.indexWhere((e) => e.day == days[index]);
            return Container(
              ...
              child: ExpansionTile(
                ...
                children: [
                  if (scheduleIndex != -1)
                    for (int i = 0;
                        i < gateData.schedules[scheduleIndex].actions.length;
                        i++)
                      ListTile(
                        contentPadding:
                            const EdgeInsets.only(left: 65, right: 65),
                        title: Text(
                            gateData.schedules[scheduleIndex].actions[i].time,
                            style: const TextStyle(
                                fontSize: 20, color: Colors.black)),
                        subtitle: Text(
                          gateData.schedules[scheduleIndex].actions[i].action
                              ? "on".tr
                              : "off".tr,
                        ),
                        trailing: IconButton(
                            onPressed: () => context.read<MainBloc>().add(
                                MainEventDeleteSchedule(
                                    ...
                                    id: gateData.id,
                                    day: days[index],
                                    actionId: gateData.schedules[scheduleIndex]
                                        .actions[i].id)),
                            icon: const Icon(
                              Icons.delete,
                              size: 26,
