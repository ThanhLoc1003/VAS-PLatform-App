...
class ScheduleAircon extends StatefulWidget {
  const ScheduleAircon({super.key, required this.token});
  final String token;

  @override
  State<ScheduleAircon> createState() => _ScheduleAirconState();
}

class _ScheduleAirconState extends State<ScheduleAircon> {
      ...
      children: [
        ...
        for (int index = 0; index < days.length; index++)
          Builder(builder: (context) {
            int scheduleIndex =
                airconData.schedules.indexWhere((e) => e.day == days[index]);
            return Container(
              ...
              child: ExpansionTile(
                ...
                children: [
                  if (scheduleIndex != -1)
                    for (int i = 0;
                        i < airconData.schedules[scheduleIndex].actions.length;
                        i++)
                      ListTile(
                        contentPadding:
                            const EdgeInsets.only(left: 65, right: 65),
                        title: Text(
                            airconData.schedules[scheduleIndex].actions[i].time,
                            style: const TextStyle(
                                fontSize: 20, color: Colors.black)),
                        subtitle: Text(
                          airconData.schedules[scheduleIndex].actions[i].action
                              ? "on".tr
                              : "off".tr,
                        ),
                        trailing: IconButton(
