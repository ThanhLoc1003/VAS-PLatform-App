...
class ScheduleLock extends StatefulWidget {
  const ScheduleLock({super.key, required this.token});
  final String token;

  @override
  State<ScheduleLock> createState() => _ScheduleLockState();
}

class _ScheduleLockState extends State<ScheduleLock> {
      ...
      children: [
        ...
        for (int index = 0; index < days.length; index++)
          Builder(builder: (context) {
            int scheduleIndex =
                lockData.schedules.indexWhere((e) => e.day == days[index]);
            return Container(
              ...
              child: ExpansionTile(
                ...
                children: [
                  if (scheduleIndex != -1)
                    for (int i = 0;
                        i < lockData.schedules[scheduleIndex].actions.length;
                        i++)
                      ListTile(
                        contentPadding:
                            const EdgeInsets.only(left: 65, right: 65),
                        title: Text(
                            lockData.schedules[scheduleIndex].actions[i].time,
                            style: const TextStyle(
                                fontSize: 20, color: Colors.black)),
                        subtitle: Text(
                          lockData.schedules[scheduleIndex].actions[i].action
                              ? "on".tr
                              : "off".tr,
                        ),
                        trailing: IconButton(
                            onPressed: () => context.read<MainBloc>().add(
                                MainEventDeleteSchedule(
                                    ...
                                    id: lockData.id,
                                    day: days[index],
                                    actionId: lockData.schedules[scheduleIndex]
                                        .actions[i].id)),
                            icon: const Icon(
                              Icons.delete,
                              size: 26,
